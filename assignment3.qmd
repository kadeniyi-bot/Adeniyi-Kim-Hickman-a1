...
historical_pop <- historical_pop[1:10, -3]
historical_pop
```
...
for (i in east_grand_boul_change) {
  ...
  if (length(chiboul_tables) >= 4) {
    ...
    if (ncol(hist_population) >= 2 && nrow(hist_population) >= 11) {
      hist_population <- hist_population[1:10, -3] 
...
Some of the most common words used are Oakland, located, South Chicago, Illinois.
...
ggplot(most_used_words, aes(x = common_words, y = n)) +
  ...
  labs(title = "Top 5 Words by Location", x = "Common_Words", y = "Word_Counts")

```

**Answer**

The most common words are Park (n=74), Hyde (n=69), and Chicago (n=35).
From the graph, the similarities between word at least one of the words Hyde, Park, and Chicago came up on the three locations. For the differences, the words south was only present in Hyde park, the word was only present in Kenwood and the homes and housing was only present in the Oakland graph.
