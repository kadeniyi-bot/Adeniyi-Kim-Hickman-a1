...
FROM crime WHERE year = 2016 AND arrest = TRUE
...
SELECT EXTRACT(HOUR FROM date) AS hour, 
  COUNT(*) AS arrests_count FROM crime
...
The hour 19 (7 PM) had the highest number of arrests at about 3843.
It was followed by hours 18 (6 PM), 20 (8 PM), 21 (9 PM), and 16 (4 PM). Most of the arrests occurred in the evening hours.
...
```{sql connection = con}
SELECT year, count(*) AS arrest_by_homicide FROM crime
WHERE primary_type = 'HOMICIDE' AND arrest = TRUE
GROUP BY year ORDER BY arrest_by_homicide DESC
LIMIT 10;
```
...
The highest number of homicide arrests occurred in the early 2000s — particularly in 2001 (431 arrests), 2002 (428 arrests), and 2003 (386 arrests). Also, years such as 2020, 2022, and 2021 also recorded relatively high numbers of homicide arrests.
Overall, homicide arrests were most frequent in the early 2000s, and there was a resurgence around 2020–2022.
...
```{sql, connection=con}
SELECT year, district,  COUNT(*) AS arrests_numbers
FROM crime
WHERE year IN (2015, 2016) 
  AND arrest = TRUE
GROUP BY year, district
ORDER BY arrests_numbers DESC
LIMIT 10;
```
...
District 11 had the highest number of arrests in both 2015 (8,975) and 2016 (6,578). Districts 7, 15, 6, 25, and 4 also had high arrest counts in 2015 alone.
...
query <- "
  SELECT
    primary_type,
    COUNTIF(arrest = TRUE) AS arrests_n
  FROM crime
  WHERE district = 11 AND year = 2016
  GROUP BY primary_type
  ORDER BY arrests_n DESC
"
```
...
```{r}
res_arrest1 <- crime_query %>%
  filter(district == 11, year == 2016, arrest == TRUE) %>%
  group_by(primary_type) %>%
  summarise(arrest = n(), .groups = "drop") %>%
  arrange(desc(arrest)) %>%
  collect()
```
